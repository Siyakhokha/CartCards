...
const ButtonContainer = ({
    ...
    addLineItem({ cartId: latestCart.id, merchandiseId: variantID, quantity: 1 })
      .then(response => {
        ...
        if (errors && errors.length > 0) {
          ...
          const item = responseData?.lines?.edges?.find(item => item?.node?.merchandise?.id === variantID)
          const discount = responseData?.discountCodes?.find(codes => codes.applicable)
      ...
      .then(response => {
        ...
        if (errors && errors.length > 0) {
          ...
          const newCart = lines?.length
            ...
            const item = responseData?.lines?.edges?.find(item => item.node.id === existingLineItem.id)
            const discount = responseData?.discountCodes?.find(codes => codes.applicable)
          updateCartState(newCart, id, item, discount);
        }
      })
    ...
    createCart({ merchandiseId: variantID, quantity: 1 })
      .then(response => {
        ...
        if (errors && errors.length > 0) {
          ...
          const discount = createdCart?.discountCodes?.find(codes => codes.applicable)
  ...
  const sendMparticleEvent = (item, discount) => {
    ...
    const validDiscount = item?.node?.discountAllocations?.find(discAll => discAll?.discountedAmount?.amount > 0)
